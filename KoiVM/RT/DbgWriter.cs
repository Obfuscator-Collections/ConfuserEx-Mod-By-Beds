	...
	internal class DbgWriter
	{
		...
		internal class DbgSerializer : IDisposable
		{
			...
			internal DbgSerializer(DbgWriter dbg)
			{
				...
				writer = new BinaryWriter(new DeflateStream(new CryptoStream(stream, aes.CreateEncryptor(), CryptoStreamMode.Write), CompressionMode.Compress));
				InitStream();
			}
			...
			private void InitStream()
			{
				...
				uint docId = 0u;
				foreach (string doc in dbg.documents)
				{
					writer.Write(doc);
					docMap[doc] = docId++;
				}
			...
			{
				if (chunk == null || !dbg.entries.TryGetValue(chunk.Block, out var entryList) || chunk.Block.Content.Count == 0)
				{
					return;
				}
		...
		{
			if (!entries.TryGetValue(block, out var entryList))
			{
				List<DbgEntry> list2 = (entries[block] = new List<DbgEntry>());
				entryList = list2;
			}
